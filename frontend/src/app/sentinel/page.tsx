...
export default function SentinelPage() {
  ...
  const adaptive = intelligence?.adaptive_report;
  const crossCaseInsights: CrossCaseInsights = adaptive?.cross_case_insights || {
    effective_sources: {},
    avg_response_times: {},
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [statusRes, alertsRes, capRes, intelRes, cacheRes] = await Promise.all([
        apiClient.getSentinelStatus().catch(e => { console.error(e); return null; }),
        apiClient.getSentinelAlerts(20).catch(e => { console.error(e); return []; }),
        apiClient.getSentinelCapability().catch(e => { console.error(e); return null; }),
        fetch(`${baseUrl}/intelligence/report`).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch(`${baseUrl}/cache/stats`).then(r => r.ok ? r.json() : null).catch(() => null),
      ]);
      ...
      if (statusRes) {
        ...
        // If statusRes is null but loading finished, it's likely a connection error
        setError("Could not connect to Sentinel backend. Please ensure the server is running.");
      }
      ...
      setError("An unexpected error occurred while fetching system metrics.");
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  if (error) {
    return (
      <div className="flex h-full flex-col items-center justify-center">
        <div className="glass rounded-3xl p-12 text-center max-w-md border-red-500/20">
          <AlertCircle size={40} className="text-red-400 mx-auto mb-6" />
          <h2 className="text-lg font-light tracking-[0.15em] text-red-300 uppercase mb-3">
            System Error
          </h2>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={fetchData}
            className="px-6 py-2.5 rounded-xl bg-gray-800 text-gray-300 text-xs font-mono uppercase tracking-wider hover:bg-gray-700"
          >
            Retry Connection
          </motion.button>
        </div>
  ...
  const domainExpertise = intelligence?.adaptive_report?.domain_expertise || intelligence?.memory?.domain_stats || {};
  const totalCases = intelligence?.adaptive_report?.total_cases || intelligence?.memory?.total_cases || 0;
              ...
              <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-8 pt-6 border-t border-white/5 relative z-10">
                    {[
                      { label: 'Total Cases', value: totalCases, color: 'text-indigo-400' },
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(domainExpertise).map(([domain, data]: [string, any]) => {
                        const count = data.count || data.case_count || 0;
                        const successRate = data.avg_quality || data.success_rate || 0;
                        return (
                          <motion.div
                            key={domain}
                          ...
                          >
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <span className="text-[10px] font-mono text-gray-500">{count} cases</span>
                            </div>
                            <div className="space-y-2">
                              <div className="flex justify-between text-xs font-mono">
                                <span className="text-gray-500">Quality / Success</span>
                                <span className="text-gray-300">{Math.round(successRate * 100)}%</span>
                              </div>
                              <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                                <motion.div
              ...
              {adaptive?.system_personality && (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(adaptive!.system_personality!).map(([key, val]: [string, any]) => (
